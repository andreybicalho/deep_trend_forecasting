...
def get_dataset(**kwargs):
    ...
    candlestick = binance.get_historical_candlestick(symbol=args.symbol,
                                                     interval=args.interval, range_start=args.range_start,
...
def build_model(time, series, split_time, window_size=30, epochs=30, batch_size=64, shuffle_buffer_size=1000):
    ...
    if args.verbose:
        show_training_results(history=history, model=model, series=series, time_valid=time_valid, x_valid=x_valid,
...
def get_model():
    ...
    return build_model(time=time, series=series, split_time=args.split_time, window_size=args.window_size,
                       epochs=args.epochs, batch_size=64, shuffle_buffer_size=1000)
